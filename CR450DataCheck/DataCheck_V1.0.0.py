...
def merge_dat_files(folder_path=None, output_file='mergedata.dat'):
    # 如果 folder_path 为 None，则使用当前文件夹
    if not os.path.exists(folder_path):
        save_print_to_txt('./log.txt', '文件路径不存在')
        return None
    # 如果输出文件已存在，则删除
    ...
    # 如果没有找到 .dat 文件，打印提示并返回
    if not dat_files:
        save_print_to_txt('./log.txt', f"{folder_path}文件夹下没有 .dat 数据。")
        print(f"{folder_path}文件夹下没有 .dat 数据。")
        return None
        ...
        for i, dat_file in enumerate(dat_files):
            ...
            with open(file_path, 'rb') as infile:
                if i == 0:
                    ...
                    infile.seek(16)
                    outfile.write(infile.read())
    save_print_to_txt('./log.txt', f"合并完成，生成文件: {output_path}")
...
class DataCheck():
    ...
    def data_process(self):
        ...
        for obj in cfg_data['obj']:
            ...
            data_file = merge_dat_files(folder_path=folder)
            ...
            frame_data = read_binary_file(data_file, frameoffset, timeoffset, starttime, endtime, framesize)
            if not frame_data:
                # 如果 frame_data 为空，执行相应的操作
                save_print_to_txt('./log.txt', '++++++++所给时间段的数据为空+++++++++')
                print("帧列表为空")
                continue

            if obj['obj1'] == "WNDS":
                result = WNDS_data_judge(frame_data, timeoffset)
                save_variables_to_file(result, starttime, "WNDS", file_path=folder)
            elif obj['obj1'] == "BIDS":
                result = BIDS_data_judge(frame_data, timeoffset)
                save_variables_to_file(result, starttime, "BIDS", file_path=folder)
            elif obj['obj1'] == "GVDS":
                result = GVDS_data_judge(frame_data, timeoffset)
                save_variables_to_file(result, starttime, "GVDS", file_path=folder)
            elif obj['obj1'] == "MVDS":
                result = MVDS_data_judge(frame_data, timeoffset)
                save_variables_to_file(result, starttime, "MVDS", file_path=folder)
