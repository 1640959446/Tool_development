...
def extract_frame_time(frame, time_offset):
    """将二进制帧数据转换为UNIX时间戳
    Args: frame: 二进制数据帧，长度至少29字节
    Returns: int: UNIX时间戳（秒级）
    Raises: ValueError: 当遇到无效日期或字段越界时
    """
    # 检查帧数据长度
    if len(frame) < 255:
        raise ValueError("帧数据长度不足100字节")
...
def MVDS_data_judge(framelist, time_offset):
    ...
    for frame in framelist:
        ...
        nochuandong_cnt = 95
        for i in range(8):
            val = struct.unpack('>H', frame[152 + i * 2:152 + i * 2 + 2])[0] * 0.1
            if i % 2 == 0:
        ...
        nochuandong_cnt = 111
        for i in range(8):
            val = struct.unpack('>H', frame[168 + i * 2:168 + i * 2 + 2])[0] * 0.1
            if i % 2 == 0:
                if val > res[chuandong_cnt]:
                    res[chuandong_cnt] = val
                    res[chuandong_cnt + 1] = speed
...
class DataCheck():
    ...
    def data_process(self):
        ...
        for obj in cfg_data['obj']:
            ...
            pure_names = [p.name for p in Path(folder).rglob("*.dat")]
            if not pure_names:
                print(f"{obj['obj1']}文件夹为空")
                continue
                # 获取第一个文件名
            data_file = os.path.join(folder, pure_names[0])
            ...
            if obj['obj1'] == "WNDS":
                ...
                save_variables_to_file(result, starttime, "WNDS", file_path='./data/WNDS/')
            elif obj['obj1'] == "BIDS":
                result = BIDS_data_judge(frame_data, timeoffset)
                save_variables_to_file(result, starttime, "BIDS", file_path='./data/BIDS/')
            elif obj['obj1'] == "GVDS":
                result = GVDS_data_judge(frame_data, timeoffset)
                save_variables_to_file(result, starttime, "GVDS", file_path='./data/GVDS/')
            elif obj['obj1'] == "MVDS":
                result = MVDS_data_judge(frame_data, timeoffset)
                save_variables_to_file(result, starttime, "MVDS", file_path='./data/MVDS/')
